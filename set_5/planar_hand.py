...
from grasp_metrics import achieves_force_closure, get_perpendicular2d
...
class HandController(LeafSystem):
    ...
    def ComputeControlInput(self, x, t):
        ...
        betas = b
        for i in range(len(self.grasp_points)):
            b = mp.NewContinuousVariables(2, "betas_%d"%k) #pair of betas for each contact point
            betas = np.vstack((betas,b))    
            mp.AddLinearConstraint(betas[i,0]>=0).evaluator().set_description("beta_0 greater than 0 for %d"%i)
            mp.AddLinearConstraint(betas[i,1]>=0).evaluator().set_description("beta_1 greater than 0 for %d"%i)        
        ...
        for i in range(1, len(self.grasp_points)):
            n = grasp_normals_world_now.T[:,0:2] #row=contact point? transpose then index
...
#             print(dyn.shape)
        ...
        Kd = 1.0
        control_period = 0.0333
        next_q = q+v.dot(control_period)+np.dot(qdd.dot(control_period**2),.5)
        next_q_dot = v+qdd.dot(control_period)
        mp.AddQuadraticCost(Kp*(qdes-next_q).dot((qdes-next_q).T)+Kd*(next_q_dot).dot(next_q_dot.T))
        Kp_ext = 100.
        mp.AddQuadraticCost(Kp_ext*(qdes-next_q)[-3:].dot((qdes-next_q)[-3:].T))
