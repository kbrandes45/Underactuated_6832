...
class Hopper2dController(VectorSystem):
    ...
    def x_touchdown(self, xd, theta, thetad, l):
        k1 = .50
        k2 = .50
        k3 = .50
        theta_desired = 0.0
        #print(xd, theta, thetad, l)
        x_err = k1*(xd-self.desired_lateral_velocity)+k2*(theta - theta_desired)+k3*thetad
        #print(x_err)
        r2 = self.l_max-l
        r1 = l
        w=1.0
        numerator = w*r2*self.m_b*np.sin(theta)+(self.m_b+self.m_f)*x_err
        denom = r1*self.m_f+w*self.m_b
        #print(denom)
        #print(numerator/(denom*1.0))
        x_td = numerator/(denom*1.0)
        return x_td
    
    def beta_des(self, x_td):
#         print(x_td)
        inners = x_td
        if inners<-1:
            inners = -1.0
    ...
    def ChooseThighTorque(self, X):
        ...
        x_err = k1*(xd-self.desired_lateral_velocity)+k2*(theta - theta_desired)+k3*thetad
        x_td = ((r2*self.m_b*np.sin(theta)+l*(self.m_b+self.m_f)*x_err)/(r1*self.m_b))#+1.0*xd/2.0
        beta_desired = self.beta_des(x_td)

        if in_contact:
            torque = kp*(theta-theta_desired)+kd*thetad

        else:
            torque = kpa*(beta_desired - (alpha+theta))
